...
export default function Sidebar({ onSelecionarUsuario, usuarioSelecionado }) {
  const [peers,     setPeers]     = useState([])
  const [status,    setStatus]    = useState(null)

  // Polling de status e peers a cada 5s
  useEffect(() => {
    function atualizar() {
      getStatus().then(setStatus).catch(() => {})
      getPeers().then(data => {
        // Transforma "127.0.0.1:8082" em objeto { nome, chave }
        const lista = (data.peers || []).map(p => ({
          nome:  p,
          chave: p, // chave = endereço IP:porta (mock — será chave pública real futuramente)
        }))
        setPeers(lista)
      }).catch(() => {})
    }
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <span style={{
          fontFamily: 'var(--font)', fontSize: '18px',
          color: '#F8C800',
      ...
      <ul style={{ listStyle: 'none', padding: '4px 0', overflowY: 'auto', flex: 1 }}>
        {peers.length === 0 && (
          <li style={{ padding: '8px 10px', fontFamily: 'var(--font)', fontSize: '6px', color: 'rgba(255,255,255,0.3)' }}>
            Nenhum peer
          </li>
        )}
        {peers.map(u => (
    ...
    <div>
      ...
      {resultado && (
        <div style={{ marginTop: '8px', textAlign: 'center' }}>
          <img
            src={getSpriteUrl(resultado.rewardId)}
            alt={resultado.rewardPokemon}
            style={{ width: '48px', height: '48px', imageRendering: 'pixelated' }}
          />
          <p style={{ fontFamily: 'var(--font)', fontSize: '6px', color: '#F8C800', lineHeight: 1.8 }}>
